...
const QUIZ = {
  ...
  getQuestions(gameState) {
    ...
    // Toujours 1 Corey + 2 sérieuses — ordre aléatoire
    const corey   = pool.filter(q => q.categorie === 'COREY');
    const serieux = pool.filter(q => q.categorie !== 'COREY');
    const sel = [];
    if (corey.length)       sel.push(corey[0]);
    if (serieux.length)     sel.push(serieux[0]);
    if (serieux.length > 1) sel.push(serieux[1]);

    // Mélanger l'ordre des 3
