...
const ACHIEVEMENTS = {
  ...
  liste: [
    ...
    { id:'failure',       icon:'🗣', titre:"FAILURE TO COMMUNICATE",      desc:"4 mauvaises ventes de suite.",                    categorie:'CINÉMA', secret:false, check:(gs)=>(gs.mauvaises_ventes_suite||0)>=4 },
    ...
    { id:'auteur',        icon:'🎭', titre:'AUTEUR',                      desc:'5 notes supérieures à 8 consécutives.',           categorie:'CINÉMA', secret:false, check:(gs)=>{ const n=gs.notesHistory||[];let s=0;for(const x of n){if(x>=8){s++;if(s>=3)return true;}else s=0;}return false; } },
    { id:'underground',   icon:'⚡', titre:'UNDERGROUND',                 desc:'Distribuer 10 cassettes noires.',                categorie:'CINÉMA', secret:false, check:(gs)=>(gs.noires_distribuees||0)>=4 },
    ...
    { id:'dead_alive',    icon:'💪', titre:'DEAD OR ALIVE',               desc:'Le musclé est ravi 3 fois.',                     categorie:'CINÉMA', secret:false, check:(gs)=>(gs.client_muscle_positif||0)>=2 },
    ...
    { id:'yippee',        icon:'🎄', titre:'YIPPEE-KI-YAY',               desc:'Die Hard loué 5 fois.',                          categorie:'CINÉMA', secret:false, check:(gs)=>(gs.film_die_hard_count||0)>=2 },
    { id:'be_kind',       icon:'📼', titre:'BE KIND REWIND',              desc:'20 cassettes rendues non rembobinées.',          categorie:'CINÉMA', secret:false, check:(gs)=>(gs.retour_non_rembobinee_count||0)>=3 },
    { id:'they_live',     icon:'🕶', titre:'THEY LIVE',                   desc:'Le concurrent vient 10 fois sans rien trouver.', categorie:'CINÉMA', secret:true,  check:(gs)=>(gs.client_concurrent_count||0)>=3 },
    { id:'bon_brute',     icon:'🤠', titre:'LE BON LA BRUTE ET LE TRUAND',desc:'Le cowboy vient 5 fois.',                       categorie:'CINÉMA', secret:false, check:(gs)=>(gs.client_cowboy_count||0)>=2 },
    { id:'hit_it',        icon:'🎬', titre:'HIT IT',                      desc:'Sam répète ses répliques. 5 visites.',           categorie:'CINÉMA', secret:false, check:(gs)=>(gs.client_sam_count||0)>=2 },
    ...
    { id:'pretty_w',      icon:'🌹', titre:'PRETTY WOMAN',                desc:"Ramon est venu 3 fois. Corey compte.",           categorie:'COREY', secret:false, check:(gs)=>(gs.client_ramon_count||0)>=3 },
