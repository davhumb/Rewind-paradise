...
const CASSETTES_DATA = [
...

  {
    ...
    semaine_min: 2,
    difficulte: 1,
    images: ['micrometrages/mc-1.png','micrometrages/mc-2.png','micrometrages/mc-3.png','micrometrages/mc-4.png','micrometrages/mc-5.png'],
    segments: [
      { zone:'bleu',   label:'MISE EN PLACE', image:'micrometrages/mc-1.png' },
      { zone:'bleu',   label:'MISE EN PLACE', image:'micrometrages/mc-2.png' },
      { zone:'orange', label:'TENSION',       image:'micrometrages/mc-3.png' },
      { zone:'rouge',  label:'BASCULE',       image:'micrometrages/mc-4.png' },
      { zone:'vert',   label:'RÉSOLUTION',    image:'micrometrages/mc-5.png' }
    ],
    coupesIdeal: [2],
    coupeAcceptable: null,
    pasDeCoupe: false,
...
function getCassetteAleatoire(semaine) {
  ...
  if (!dispo.length) return CASSETTES_DATA[0];
  
  // Récupérer les cassettes déjà jouées depuis gameState
  let dejouees = [];
  try {
    const save = localStorage.getItem('rewindParadise_save');
    if (save) {
      const gs = JSON.parse(save);
      dejouees = gs.cassettes_jouees || [];
    }
  } catch(e) {}
  
  // Préférer celles pas encore jouées
